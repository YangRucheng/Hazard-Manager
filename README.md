...
隐患记录闭环管理平台：隐患台账、系统管理（责任单位/隐患类型/附件管理）、图片上传（摘要去重 + 缩略图预览）、JWT 鉴权（管理端，架构预留小程序端）。响应式：桌面端可折叠侧栏，移动端抽屉导航。
...
└── web/                      # Vue3 + Vite 前端
    └── src/
        ...
        ├── views/            # 登录 / 工作台 / 隐患列表 / 隐患表单 / 系统管理（责任单位、隐患类型、附件管理）
...
- **责任单位无排序**：`responsible_units` 不含 `sort` 列，列表与隐患表单下拉均按创建顺序（`id` 升序）返回；旧库遗留 `sort` 列在服务启动时自动删除。
- **隐患类型**：`hazard_types` 每行 = 一个「大类(`major`)+小类(`minor`)」组合，**无父级引用、无排序/启停**，同一组合唯一；新增时大类用「下拉输入框」可复用已有大类或直接输入新大类。隐患记录只存 `type_id` 指向该组合行（不再有 `category_id`）。
...
- **删除保护**：单位/类型被隐患引用（含软删除历史）时返回 409；被引用的隐患类型**只允许修改、不允许删除**。旧版「单表两级」结构由服务启动时自动迁移为新结构。
...
集成测试覆盖：默认值、检查日期+7 联动、单位→责任人联动、类型存在性校验、非法状态拦截、单位不存在 404/422、统计。

## 验收清单

- [ ] 未登录访问受保护接口返回 401；错误密码与正确密码登录行为正确；`/auth/me` 返回 admin。
- [ ] 新增隐患：默认值（华星现场 / 今天 / 电气自查 / 检查日期+7 天 / 复查=检查人员）、选责任单位自动带出责任人、按大类分组选择隐患类型（=大类+小类组合）、多图上传去重（同图同 uuid、磁盘无重复文件）、缩略图可预览。
- [ ] 列表：筛选（状态/等级/类型/区域/单位/关键字/日期范围）、分页、图片列缩略图、逾期标记、编辑/删除。
- [ ] 系统管理：责任单位 / 隐患类型 / 附件管理三个子 tab；责任单位与隐患类型删除按钮位于编辑弹窗内，被引用删除返回冲突提示（被引用类型只能修改）；附件管理分页展示所有图片与引用数，可仅看未引用，删除被引用附件返回 409。
- [ ] UI：左侧导航（工作台/隐患管理/系统管理，白底可折叠、移动端抽屉）、顶部右上 admin 下拉退出登录、全站蓝色系、警示色仅用于状态/等级标签、手机端响应式布局。